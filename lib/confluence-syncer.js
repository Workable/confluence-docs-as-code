...
function diff(localPages, remotePages) {
    ...
    const remoteIndex = util.keyBy(remotePages, 'meta.path');
    for (let localPage of localPages) {
        const remotePage = remoteIndex[localPage.meta.path];
        if (!remotePage) {
            // not exist on remote -> create
            results.create.push(localPage);
        } else if (config.confluence.forceUpdate || versionChange(remotePage) || localPage.meta.sha !== remotePage.meta.sha) {
    ...
    const localIndex = util.keyBy(localPages, 'meta.path');
    for (let remotePage of remotePages) {
        if (!localIndex[remotePage.meta.path]) {
