...
async function syncHome(repo, siteName, readMe, pageRefs) {
    ...
    if (existingPage) {
        ...
        if (config.confluence.force_update || existingPage.sha !== homeMeta.sha) {
...
function diff(localPages, remotePages) {
    ...
    for (let localPage of localPages) {
        ...
        } else if (config.confluence.force_update || localPage.sha !== remotePage.sha) {
            // if force_update or exist with different sha, then update the page
