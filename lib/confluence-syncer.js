...
export async function sync() {
    logger.info(`Action Version: ${config.version}`);
    logger.info(`GitHub Ref: ${config.github.refName}`);
    logger.info(`GitHub SHA: ${config.github.sha}`);
    try {
...
async function syncHome(repo, siteName, readMe, pageRefs) {
    ...
    if (existingPage) {
        ...
        if (config.confluence.forceUpdate || existingPage.sha !== homeMeta.sha) {
...
function diff(localPages, remotePages) {
    ...
    for (let localPage of localPages) {
        ...
        } else if (config.confluence.forceUpdate || localPage.sha !== remotePage.sha) {
