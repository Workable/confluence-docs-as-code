 ...
 * @param {LocalPage} readMe 
 * @param {AssetRenderer} renderer 
 * @returns {number} Home page id
 */
async function syncHome(repo, siteName, readMe, renderer) {
    ...
    if (existingPage) {
        ...
        if (config.confluence.forceUpdate || versionChange(existingPage) || existingPage.meta.sha !== homeMeta.sha) {
            ...
            await createAttachments(existingPage.id, attachments, renderer);
        }
        return existingPage.id;
    } else {
        const pageId = await confluence.createPage(homeTitle, homeHtml, parentPage, homeMeta);
        await createAttachments(pageId, attachments, renderer);
        return pageId;
    }
}
...
async function syncPages(home, localPages, renderer) {
    ...
    await update(differences.update, home, renderer);
    // create added pages
    await create(differences.create, home, renderer);
}

/**
 * 
 * @param {number} home - The id of the home page 
 * @param {Array<LocalPage>} localPages - Array of pages to be created
 * @param {AssetRenderer} renderer - `AssetRenderer` instance 
 */
async function create(localPages, home, renderer) {
    for (let page of localPages) {
        page.render(renderer);
        const pageId = await confluence.createPage(page.title, page.html, home, page.meta);
        await createAttachments(pageId, page.attachments, renderer);
 ...
 * 
 * @param {number} home - The id of the home page 
 * @param {Array<LocalPage>} localPages - Array of pages to be updated
 * @param {AssetRenderer} renderer - `AssetRenderer` instance 
 */
async function update(localPages, home, renderer) {
    for (let page of localPages) {
        page.render(renderer);
        await createAttachments(page.id, page.attachments, renderer);
        await confluence
            .updatePage(page.id, page.version, page.title, page.html, home, page.meta)
            .then(() => {
                logger.debug(
...
async function createAttachments(pageId, attachments, renderer) {
    for (const attachment of attachments) {
        if (attachment instanceof Image) {
            await createImage(pageId, attachment);
        } else if (attachment instanceof Graph) {
            await createGraph(pageId, attachment, renderer);
        }
    }
}
...
/**
 * 
 * @param {number} pageId - The id of the page to attach the graph 
 * @param {Graph} graph - The graph to be created
 * @param {AssetRenderer} renderer
 */
async function createGraph(pageId, graph, renderer) {
    const attachment = await graph.render(renderer);
    if (!attachment) {
