...
export async function sync() {
    try {
        ...
        const renderer = new PageRenderer(config, pageRefs);
        const home = await syncHome(repo, siteName, readMe, renderer);
        await syncPages(home, localPages, renderer);
...
async function create(localPages, root, renderer) {
    for (let page of localPages) {
        renderer.render(page);
        const pageId = await confluence.createPage(page.title, page.html, root, page.meta);
        await createAttachments(pageId, page.attachments);
        logger.debug(`Created Page: [${pageId}] ${page.title}`);
    }
}

async function update(localPages, root, renderer) {
    for (let page of localPages) {
        renderer.render(page);
        await createAttachments(page.id, page.attachments);
        await confluence
            .updatePage(page.id, page.version, page.title, page.html, root, page.meta)
            .then(() => {
                logger.debug(
